package uz.developer.zohidjon.backend.service;
...
import uz.developer.zohidjon.backend.mapper.BookMapper;
import uz.developer.zohidjon.backend.repository.BookRepository;
import uz.developer.zohidjon.backend.common.PageResponse;
import uz.developer.zohidjon.backend.dto.request.BookRequest;
import uz.developer.zohidjon.backend.dto.response.BookResponse;
import uz.developer.zohidjon.backend.dto.response.BorrowedBookResponse;
import uz.developer.zohidjon.backend.entity.Book;
...
import uz.developer.zohidjon.backend.entity.BookTransactionHistory;
import uz.developer.zohidjon.backend.repository.BookTransactionHistoryRepository;
...
public class BookService {
    ...
    public PageResponse<BookResponse> findAllBooksByOwner(int page, int size, Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        Page<Book> books = bookRepository.findAll((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("owner").get("id"), user.getId()), pageable);
        List<BookResponse> booksResponse = books.stream()
                .map(bookMapper::toBookResponse)
                .toList();
        return new PageResponse<>(
